    ...
    if (!hparams.vocab_only) {
        ...
        for (auto & backend : backends) {
            ...
            if (reg) {
                ...
                // Volta-only: opt-in prefill GEMM weight reuse (see ggml-cuda.cu).
...
llama_context * llama_init_from_model(
    ...
    // TurboQuant cache types require flash attention - auto-enable even when explicitly
    // disabled, so this has to run before the generic quantized-V check below, which
    // would otherwise reject the combination outright.
    ...
    if (params.flash_attn_type != LLAMA_FLASH_ATTN_TYPE_DISABLED && ggml_is_quantized(params.type_k)) {
        ...
        for (uint32_t il = 0; il < model->hparams.n_layer(); ++il) {
            ...
            // Turbo types zero-pad heads to the next multiple of 128 in llama-kv-cache.cpp
    ...
    if (params.flash_attn_type != LLAMA_FLASH_ATTN_TYPE_DISABLED && ggml_is_quantized(params.type_v)) {
        ...
        for (uint32_t il = 0; il < model->hparams.n_layer(); ++il) {
            ...
            // Turbo types zero-pad; MLA has no separate V cache (V is a view of K)
