...
struct ggml_backend_cuda_comm_context {
    ...
    // Set by the init chain (comm_init_{nccl, internal, none, p2p}) to one of
    // try_allreduce_{nccl, internal, butterfly, p2p}.  nccl needs `comms`,
    // internal needs `ar_pipeline`, p2p needs `ar_p2p` plus a `fallback_allreduce`,
    // butterfly needs nothing.  Per-call
...
// Top-level init.  Picks one of the four init paths based on
