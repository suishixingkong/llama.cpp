...
static constexpr __host__ __device__ fattn_mma_config ggml_cuda_fattn_mma_get_config_rdna(const int DKQ, const int DV, const int ncols) {
    ...
    // Fall back to the ampere config rather than upstream's zero-sentinel: the
    // template instances do constexpr arithmetic on the returned config
    // (np = nwarps * cols_per_warp / ncols, ...) and a sentinel with nwarps == 0
    // is a compile-time division by zero.
    return ggml_cuda_fattn_mma_get_config_ampere(DKQ, DV, ncols);
}
...
static constexpr __host__ __device__ fattn_mma_config ggml_cuda_fattn_mma_get_config_cdna(const int DKQ, const int DV, const int ncols) {
    ...
    // D=640: padded turbo KV cache (ncols2=16 only, same tiles as D=576)
...
// Volta: the exact-geometry D=192/DV=128 4x16 layout wants a 64-row KV batch rather
// than the config table's value. Host and device variants must stay in sync because
// the launcher's grid is derived from the host value and the kernel reads the device one.
static __host__ int ggml_cuda_fattn_mma_get_nbatch_fa_layout(
        const int DKQ, const int DV, const int ncols1, const int ncols2, const int cc) {
    if (volta_mma_available(cc) && DKQ == 192 && DV == 128 && ncols1 == 4 && ncols2 == 16) {
        return 64;
    }
    return ggml_cuda_fattn_mma_get_nbatch_fa(DKQ, DV, ncols1*ncols2, cc);
}

static constexpr __device__ int ggml_cuda_fattn_mma_get_nbatch_fa_layout(
        const int DKQ, const int DV, const int ncols1, const int ncols2) {
#if defined(VOLTA_MMA_AVAILABLE)
    if (DKQ == 192 && DV == 128 && ncols1 == 4 && ncols2 == 16) {
        return 64;
    }
#endif // defined(VOLTA_MMA_AVAILABLE)
    return ggml_cuda_fattn_mma_get_nbatch_fa(DKQ, DV, ncols1*ncols2);
}
...
template<int DKQ, int DV, int ncols1, int ncols2, int nwarps, int nbatch_K2_override, bool forward_K,
...
static __device__ __forceinline__ void flash_attn_ext_f16_iter(
    ...
    constexpr int  nbatch_fa       = ggml_cuda_fattn_mma_get_nbatch_fa_layout(DKQ, DV, ncols1, ncols2);
    constexpr int  nbatch_K2_config = ggml_cuda_fattn_mma_get_nbatch_K2(DKQ, DV, ncols);
    constexpr int  nbatch_K2        = nbatch_K2_override > 0 ? nbatch_K2_override : nbatch_K2_config;
    ...
    constexpr int k0_start_max = (DKQ/2-1) - (DKQ/2-1) % nbatch_K2;
#pragma unroll
    for (int k0_iter = k0_start_max; k0_iter >= 0; k0_iter -= nbatch_K2) {
        const int k0_start = forward_K ? k0_start_max - k0_iter : k0_iter;
...
template<int DKQ, int DV, int ncols1, int ncols2, int nwarps, int nbatch_K2_override, int combine_scratch_override, bool forward_K,
    bool use_logit_softcap, bool V_is_K_view, bool use_sparse, bool needs_fixup, bool is_fixup, bool output_partial>
    ...
    if constexpr (ncols2 == 1 || use_sparse) {
        ...
        for (; kb0 < kb0_stop-1; ++kb0) {
            ...
            flash_attn_ext_f16_iter
                <DKQ, DV, ncols1, ncols2, nwarps, nbatch_K2_override, forward_K, use_logit_softcap, V_is_K_view, use_sparse, needs_fixup, is_fixup, last_iter, oob_check,
...
#pragma unroll
    ...
    constexpr int combine_scratch = combine_scratch_override > 0 ? combine_scratch_override : nbatch_combine;
...
template<int DKQ, int DV, int ncols1, int ncols2, bool use_logit_softcap, bool V_is_K_view, bool use_sparse, bool output_partial = false>
...
static __global__ void flash_attn_ext_f16(
    ...
    while (kbc < kbc_stop && kb0_stop == iter_k) {
        ...
        if (kb0_start == 0) {
            ...
            flash_attn_ext_f16_process_tile<DKQ, DV, ncols1, ncols2, nwarps, -1, -1, false, use_logit_softcap, V_is_K_view, use_sparse, needs_fixup, is_fixup, output_partial>
...
// Volta-only compact specialization: same MMA arithmetic as flash_attn_ext_f16 but with a
// smaller combine scratch (64 instead of 128 half2) and a 96-wide K tile, which pushes the
// dynamic shared memory down to 48 KiB so the D=256 tile fits the sm70 occupancy target.
// It also walks K forwards so the K/V tiles are read in the order they are consumed.
template<int DKQ, int DV, int ncols1, int ncols2, bool use_logit_softcap, bool V_is_K_view>
__launch_bounds__(ggml_cuda_fattn_mma_get_nthreads(DKQ, DV, ncols1*ncols2), ggml_cuda_fattn_mma_get_occupancy(DKQ, DV, ncols1*ncols2))
static __global__ void flash_attn_ext_f16_volta_compact(
        const char * Q_ptr,
        const char * K_ptr,
        const char * V_ptr,
        const char * mask_ptr,
        ...
        const int32_t ne10, const int32_t ne11, const int32_t ne12, const int32_t ne13,
                            ...
                            const int32_t ne31, const int32_t ne32, const int32_t ne33,
                            const int32_t nb31, const int32_t nb32, const int64_t nb33) {
    static_assert(DKQ == 256 && DV == 256 && ncols1 == 32 && ncols2 == 2, "compact Volta FA specialization only");
    ggml_cuda_pdl_sync(); // TODO optimize placement
    ...
    while (kbc < kbc_stop && kb0_stop == iter_k) {
        ...
        if (kb0_start == 0) {
            constexpr bool needs_fixup = false; // CUDA block is working on an entire tile.
            flash_attn_ext_f16_process_tile<DKQ, DV, ncols1, ncols2, nwarps, 96, 64, true, use_logit_softcap, V_is_K_view, false, needs_fixup, is_fixup, false>
...
static void ggml_cuda_flash_attn_ext_mma_f16_case_impl(
    ...
    const int  nbatch_fa      = ggml_cuda_fattn_mma_get_nbatch_fa_layout(DKQ, DV, ncols1, ncols2, cc);
    ...
    // Volta only: a separate compact specialization so the generic FA kernel symbols and
    // launch paths stay unchanged. Skipped for the partial-output variant, which the
    // compact kernel does not implement.
    if constexpr (DKQ == 256 && DV == 256 && ncols1 == 32 && ncols2 == 2 && !output_partial) {
        ...
        if (use_volta_2cta) {
            // Q: 64*(128+4), K: 32*(96+4), mask: 32*(16+4) half2 values.
            constexpr size_t nbytes_shared_compact = 49152;
            float logit_softcap;
            memcpy(&logit_softcap, (const float *) KQV->op_params + 2, sizeof(float));
#if defined(GGML_USE_HIP)
            using fattn_kernel_ptr_compact_t = const void *;
#else
            using fattn_kernel_ptr_compact_t = fattn_kernel_t;
#endif
            fattn_kernel_t fattn_kernel_compact;
            if (logit_softcap == 0.0f) {
                constexpr bool use_logit_softcap = false;
                fattn_kernel_compact = flash_attn_ext_f16_volta_compact<DKQ, DV, ncols1, ncols2, use_logit_softcap, V_is_K_view>;
#if !defined(GGML_USE_MUSA)
                static bool shared_memory_limit_raised[GGML_CUDA_MAX_DEVICES] = {false};
                if (!shared_memory_limit_raised[id]) {
                    CUDA_CHECK(cudaFuncSetAttribute(reinterpret_cast<fattn_kernel_ptr_compact_t>(fattn_kernel_compact),
                        cudaFuncAttributeMaxDynamicSharedMemorySize, nbytes_shared_compact));
            ...
            launch_fattn<DV, ncols1, ncols2>
                (ctx, dst, fattn_kernel_compact, nwarps, nbytes_shared_compact, nbatch_fa, true, true, true, false,
                 warp_size_host, partial_dst, partial_meta);
            return;
        }
    }
    ...
    if (logit_softcap == 0.0f) {
        ...
        if constexpr (!output_partial && ggml_cuda_flash_attn_ext_mma_f16_may_use_sparse(DKQ, DV, ncols1, ncols2)) {
            if (ggml_cuda_flash_attn_ext_mma_f16_shall_use_sparse(ctx, dst)) {
                constexpr bool use_sparse_kernel = true;
                fattn_kernel = flash_attn_ext_f16<DKQ, DV, ncols1, ncols2, use_logit_softcap, V_is_K_view, use_sparse_kernel, output_partial>;
                use_sparse = true;
    ...
    launch_fattn<DV, ncols1, ncols2>
        (ctx, dst, fattn_kernel, nwarps, nbytes_shared_total, nbatch_fa, true, true,
         !output_partial, use_sparse, warp_size_host, partial_dst, partial_meta);
}
