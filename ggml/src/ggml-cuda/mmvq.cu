...
static constexpr __host__ __device__ int calc_nwarps(ggml_type type, int ncols_dst, mmvq_parameter_table_id table_id, bool small_k = false, bool halve_iters = false) {
    ...
    if (table_id == MMVQ_PARAMETERS_VOLTA) {
        // sm70 (Tesla V100) tg tuning: for the batch-1 decode path, K-quant
        // vec_dot at nwarps=2 beats the GENERIC nwarps=4 (measured +1.8% tg on
        // Qwen 27B Q4_K); wider (8) and narrower (1) both regress. Non-K-quant
        // and multi-column paths keep the GENERIC values. Mirrors TURING's
        // ncols_dst==1 split, which is the closest tuned architecture.
        if (ncols_dst == 1) {
            switch (type) {
                case GGML_TYPE_Q2_K:
                case GGML_TYPE_Q3_K:
...
static constexpr __host__ __device__ int calc_rows_per_block(int ncols_dst, int table_id, bool small_k = false, int nwarps = 1) {
    if (table_id == MMVQ_PARAMETERS_GENERIC || table_id == MMVQ_PARAMETERS_GCN || table_id == MMVQ_PARAMETERS_VOLTA || table_id == MMVQ_PARAMETERS_TURING || table_id == MMVQ_PARAMETERS_GB10) {
        switch (ncols_dst) {
            case 1:
